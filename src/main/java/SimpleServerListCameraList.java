...
public class SimpleServerListCameraList {
    public static void main(String args[]) {
        String serverPort = "8080";
        SimpleServerListCameraList app = new SimpleServerListCameraList();
    ...
    private void registerTaskDef(String port) {
        try {
            ...
            InputStream stream = SimpleServerListCameraList.class.getResourceAsStream("/task/inventoryLoadingTasks.json");
    ...
    private void createWorkflowDef(String port) {
        try {
            Client client = Client.create();
            InputStream stream = SimpleServerListCameraList.class.getResourceAsStream("workflow/serversCameras.json");
    ...
    private void startWorkflow(String port) {
        try {
            ...
            client.resource("http://localhost:" + port + "/api/workflow/inventoryDataLoader").type(MediaType.APPLICATION_JSON).post(input);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
